...
def archivistGetsLocales() -> dict[str, LocaleIn]:
	...
	You can use this function to obtain the complete mapping of supported locale identifiers to `LocaleIn` [1] instances
...
def archivistGetsWeights() -> dict[str, WeightIn]:
	...
	You can use this function to obtain the complete mapping of supported weight identifiers to `WeightIn` [1] instances
...
def archivistMakesFilenameStem(fontFamily: str | None = None, locale: str | None = None, style: str | None = None, weight: str | None = None, separator: str = '.') -> str:
	...
	You can use this function to create standardized filename stems from font components. The function filters out `None` values
...
def archivistGetsSubsetCharacters() -> dict[identifierDotAttribute, dict[str, list[int]]]:
	...
	You can use this function to load glyph IDs and Unicode codepoints for each Source Han Mono locale and style combination. The
...
# TODO Create an automated way to update this data.
def archivistGetsUnicodeFiraCode() -> frozenset[int]:
	"""Return the set of Unicode codepoints present in Fira Code fonts.

	(AI generated docstring)

	You can use this function to obtain the complete set of Unicode codepoints supported by Fira Code fonts [1]. The returned set
	is used to filter characters when generating character subsets for CID fonts.

	Returns
	-------
	unicodeFiraCode : frozenset[int]
		Frozen set of Unicode codepoints present in Fira Code fonts.
...
def archivistUpdatesFontFileMetadata(pathFilename: Path, nameIDmetadata: dict[int, str]) -> Path:
	...
	You can use this function to update OpenType name records and other metadata in a font file. The function opens the font file,
...
def archivistUpdatesMetadata(ttFont: TTFont, nameIDmetadata: dict[int, str]) -> None:
	...
	You can use this function to update font version, vendor ID, and name records in an open `TTFont` [1] instance. The function
...
def archivistMakesCharacterSubsets(pathFilename: Path) -> list[Path]:
	...
	You can use this function to create `.gids` and `.unicodes` subset files from a `.UTF32-map` character map file. The function
...
def archivistMakesAllCharacterSubsets(fontFamilyCID: str = 'SourceHanMono', theLocales: Iterable[str] | None = None, theStyles: Iterable[str | None] | None = None) -> list[Path]:
	...
	You can use this function to create `.gids` and `.unicodes` subset files for all combinations of locales and styles. The
